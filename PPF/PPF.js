    // Define the data points for the PPF curve
    var ppfData = [
        { x: 0, y: 850 },
        { x: 850, y: 0 },
      ];
      ...
      // Function to draw the PPF curve and intersection lines
      function drawCurve() {
        ...
        // Redraw the axes and grid lines
        axes.appendChild(xAxis);
        axes.appendChild(yAxis);
        axes.appendChild(goodALabel);
        axes.appendChild(goodBLabel);
        ...
        // Plot the input values as a dot on the graph
        ...
        // Add the breathing effect to the dot
        ...
        // Draw dotted lines to the intersection point
      ...
      // Function to determine the dot color based on quantity values
      function getDotColor(quantityA, quantityB) {
        ...
        // Check if the dot is within or outside the PPF curve
        for (var i = 0; i < ppfData.length - 1; i++) {
          ...
          // Calculate the equation of the line segment between two PPF points
          var ppfSlope = (y2 - y1) / (x2 - x1);
          var ppfYIntercept = y1 - ppfSlope * x1;
  
          // Calculate the expected quantity of Good B for the given quantity of Good A
          var expectedQuantityB = ppfSlope * scaledQuantityA + ppfYIntercept;
  
          if (scaledQuantityB === expectedQuantityB) {
            return 'aqua'; // Point is on the PPF curve
          }
  
          if (scaledQuantityB < expectedQuantityB) {
            return 'red'; // Point is inside the PPF curve
          }
        }
  
        // Dot is outside the PPF curve
        return 'blue';
      }
  
      // Add event listener for Enter key press to trigger the "Plot PP" button click
